...
exports.init_extension = function (options, context) {
  ...
  pubsub_client.subscribeTo(options.channelPattern, function (channel, message, pattern) {
    ...
    redis_client.zadd(options.persistKeyPrefix + channel.toString(), (new Date).getTime().toString(), message.toString(), function (err, reply) {
        if (err) throw new Error(err);
        log("zadd reply from redis:" + reply);
      });

    redis_client.set(options.persistKeyPrefix + channel.toString() + ":" + message.guid, (new Date).getTime().toString(), message.toString(), function (err, reply) {
        if (err) throw new Error(err);
