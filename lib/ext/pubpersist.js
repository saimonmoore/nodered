...
exports.init_extension = function (options, context) {
  ...
  pubsub_client.subscribeTo(options.channelPattern, function (channel, message, pattern) {
    log("[ext:pubpersist] Received: " + channel.toString() + pattern ? pattern.toString() : null, message.toString());
    // redis_client.zadd(options.persistKeyPrefix + channel.toString(), (new Date).getTime().toString(), pattern ? pattern.toString() : null, message.toString(),
  });
